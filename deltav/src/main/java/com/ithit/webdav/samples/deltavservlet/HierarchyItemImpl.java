...
public abstract class HierarchyItemImpl implements com.ithit.webdav.server.HierarchyItem, Lock {
    ...
    FolderImpl getParent() throws ServerException {
        ...
        FolderImpl parent = (FolderImpl) getDataAccess().readItem(
                "SELECT ID," +
                        " Parent," +
                        " Name," +
                        " Created," +
    ...
    private void setProperty(Property prop) throws ServerException {
        int count = getDataAccess().executeInt("SELECT Count(*) FROM Property"
                + " WHERE ItemID = ?"
                + " AND Name = ?"
    ...
    private void removeProperty(String propertyName, String ns) throws ServerException {
        getDataAccess().executeUpdate("DELETE FROM Property"
                + " WHERE ItemID = ?"
                + " AND Name = ?"
    ...
    void deleteThisItem() throws ServerException {
        getDataAccess().executeUpdate("DELETE FROM VersionProperty WHERE VersionID IN (SELECT Id FROM VERSION WHERE ITEMID =  ?)", id);
        getDataAccess().executeUpdate("DELETE FROM Version WHERE ItemID = ?", id);
        getDataAccess().executeUpdate("DELETE FROM Locks WHERE ItemID = ?", id);
        getDataAccess().executeUpdate("DELETE FROM Property WHERE ItemID = ?", id);
    ...
    HierarchyItemImpl copyThisItem(FolderImpl destFolder, HierarchyItemImpl destItem, String destName)
        ...
        if (destItem == null) // insert new item
        {
            ...
            getDataAccess().executeUpdate(
                    "INSERT INTO Repository"
                            + " (ID, Name, Created, Modified, Parent, ItemType, ContentType, Content, TotalContentLength, LastChunkSaved,  CheckInDuringUnlock, AutoVersion, CheckInOnFileComplete, SerialNumber)"
                            + " SELECT ?, ?, SYSDATE, SYSDATE, ?, ItemType, ContentType, Content, TotalContentLength, LastChunkSaved, ?, AutoVersion, ?, 1"
                            + " FROM Repository"
                            + " WHERE ID = ?", destId, destName, destFolder.getId(), "N", "N", getId());
        ...
        } else // update existing destination
        {
            ...
            getDataAccess().executeUpdate("UPDATE Repository SET " +
                            "    (Modified, ItemType, ContentType, Content, LastChunkSaved, TotalContentLength, LastChunkSaved, CheckInDuringUnlock, AutoVersion, CheckinOnFileComplete, SerialNumber) = (" +
                            "     SELECT Modified,ItemType, ContentType, Content, LastChunkSaved, TotalContentLength, LastChunkSaved, ?, AutoVersion, ?, ? " +
                            "        FROM" +
                            "          Repository" +
                            "        WHERE ID = ?" +
                            " )" +
                            "WHERE" +
                            "    ID=?"
                    , "N", "N", destSN, getId(), destId);

            // remove old properties from the destination
            getDataAccess().executeUpdate("DELETE FROM Property WHERE ItemID = ?", destId);
        }
    ...
    /**
     * Generates new unique id for the version.
     *
     * @return New unique int id.
     * @throws ServerException in case of DB errors.
     */
    int genVersionId() throws ServerException {
        return getDataAccess().executeInt("select \"VERSION_SEQ\".nextval from dual");
    }
     ...
     * @throws ServerException in case of DB errors.
     */
    <T> T getDbField(String columnName, T defaultValue) throws ServerException {
        String commandText = String.format("SELECT %1s FROM Repository WHERE Id = ?", columnName);
        T res = (T) getDataAccess().executeScalar(commandText, getId());
    ...
    <T> void setDbField(String columnName, T value) throws ServerException {
        String commandText = String.format("UPDATE Repository SET %1s = ? WHERE Id = ?", columnName);
        getDataAccess().executeUpdate(commandText, value, getId());
    }
